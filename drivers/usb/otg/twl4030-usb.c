...
struct twl4030_usb {
	...
	struct delayed_work	usb_irq_delay_work; //&*&*&*SJ_20110701
};
...
static void twl4030_usb_set_mode(struct twl4030_usb *twl, int mode)
{
	...
	case T2_USB_MODE_ULPI:
      ...
      //&*&*&*Simon_20110615, set OTG as Low-Speed only in FUNC_CTRL 
/*<--LH_SWRD_CL!_andy@2011.01.19 config the usb to high speed mode !>*/
/*				
      reg = twl4030_usb_read(twl, FUNC_CTRL);	   
	   printk("[OTG]OTG FUNC_CTRL=0x%x \n",reg); 
	   twl4030_usb_set_bits(twl, FUNC_CTRL, reg|0x02);//Low-Speed
	   reg = twl4030_usb_read(twl, FUNC_CTRL);	   
	   printk("[OTG]OTG[2] FUNC_CTRL=0x%x \n",reg);	
*/
		//&*&*&*Simon_20110615 end.				
		break;
	case -1:
...
/* LH_SWRD_CL1_Henry@2011.9.30 improve USB storage connection stability (not break down frequently) -->*/
	if (status >= 0) {
		...
		//&*&*&*Simon_20110615, enable 5V when OTG-cable plug-in
...
static irqreturn_t twl4030_usb_irq(int irq, void *_twl)
{
	...
	schedule_delayed_work(&twl->usb_irq_delay_work, 1);
	return IRQ_HANDLED;
}
...
/* <-- LH_SWRD_CL1_Henry@2011.8.9 delete otg interrupt from twl4030-usb  */	
#if 0	
