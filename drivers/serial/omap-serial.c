...
//&*&*&BC1_110504: fix issue that console can not work	
...
//&*&*&BC2_110504: fix issue that console can not work	
			...
			struct ktermios *old)
{
	...
	if (termios->c_iflag & (IXON | IXOFF))
		serial_omap_configure_xonxoff(up, termios);
			...
			//&*&*&*Simon_20110314, delay 100 ms before change  baud-rate to 3000000	      
			//printk("simon: Change [uart2 to baud=%d 3000000] \n",baud);
			if(baud==3000000)
			{
				//printk("simon: Change [uart2 to baud=%d ] \n",baud);
			   mdelay(100);
			}
			//&*&*&* end Simon_20110314
...
//&*&*&BC1_110504: fix issue that console can not work	
	if (omap_is_console_port(up->pdev->id) && up->plat_hold_wakelock &&
