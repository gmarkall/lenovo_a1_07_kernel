...
static irqreturn_t twl4030_madc_irq_handler(int irq, void *_madc)
{
	...
	/* Move action of i2c read to schedule work function. 20110225 by aimar.liu */
...
/* Mark the function, because tps65921 take off current generator of BCI. 20110225 by aimar.liu */
#if 0
...
static int __init twl4030_madc_probe(struct platform_device *pdev)
{
	...
	/* Enable the VBAT prescaler in the BCI */
	ret = twl_i2c_read_u8(TWL4030_MODULE_MAIN_CHARGE,
				  &regval, TWL4030_BCI_BCICTL1);

	regval |= TWL4030_BCI_MESBAT_EN;
...
/* Add suspend/resume functions for power consumption. 20110225 by aimar.liu */
#ifdef CONFIG_PM
static int twl4030_madc_suspend(struct platform_device *dev, pm_message_t state)
{	
	int ret = 0;
	u8 regval = 0;
	struct twl4030_madc_data *madc = platform_get_drvdata(dev);
	...
	/* Disable the VBAT prescaler in the BCI */
	ret = twl_i2c_read_u8(TWL4030_MODULE_MAIN_CHARGE,
			&regval, TWL4030_BCI_BCICTL1);
...
static int twl4030_madc_resume(struct platform_device *dev)
{	
	int ret = 0;
	u8 regval = 0;
	struct twl4030_madc_data *madc = platform_get_drvdata(dev);
...
#else
#define twl4030_madc_suspend NULL
#define twl4030_madc_resume NULL
#endif /* CONFIG_PM */
...
#ifdef CONFIG_PM	
	.suspend	= twl4030_madc_suspend,
	.resume 	= twl4030_madc_resume,
#endif
	.driver		= {
		.name	= "twl4030_madc",
		.owner	= THIS_MODULE,
