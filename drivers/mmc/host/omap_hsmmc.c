...
static int omap_hsmmc_reg_get(struct omap_hsmmc_host *host)
{
	...
	if (IS_ERR(reg)) {
		...
		*/
/* <-- LH_SWRD_CL1_Henry@2011.12.1 regulator_enable ???  */
...
/* <-- LH_SWRD_CL1_Henry@2011.12.1 check null pointer as necessary  */	
	if (req == NULL)
		return -EINVAL;
	else
		{
	...
	if ((data->sg) == NULL)
		return -EINVAL;
	if ((mmc_slot(host).set_power == NULL) || (mmc_slot(host).set_sleep == NULL))
		return -EINVAL;
	if ((host->iclk == NULL) || (host->fclk == NULL))
		return -EINVAL;
...
/*LH_SWRD_CL1_andy@20110701 enable external sd card power-->*/	
/*	if(host->id == 0)
	{
	    regulator_enable(host->vcc);
	}
