...
static unsigned long old_ick;
static unsigned long old_mclk;
static unsigned long old_dpll4_m5_ck;
static unsigned long old_csi2_96m_fck;
static unsigned long old_l3_ick;
...
static irqreturn_t isp_isr(int irq, void *_pdev)
{
    ...
    //printk(KERN_DEBUG " [%s] irqstatus: %08X\n", __func__, irqstatus);
    ...
    //printk(" ******** \n\n\n[%s] irqenable: 0x%X irqStatu: 0x%X *********\n\n\n", __func__, irqenable, irqstatus);
    irqstatus &= irqenable;
...
//printk("ispccdc disable!!!\n");
	...
	if (irqstatus & CCDC_VD0) {
       ...
       // printk(KERN_DEBUG " [%s] isempty: %d bufs->queue: %d bufs->done: %d\n",
       //        __func__, ISP_BUFS_IS_EMPTY(bufs)==0, bufs->queue,  bufs->done);
...
void dumpIspPipeSettings(struct isp_pipeline *pipe){
    int __I=0;
    printk(" \n\n\t\t ISP PIPE settings: " );
    pipe->modules & OMAP_ISP_CCDC ? printk(" OMAP_ISP_CCDC "):__I;
    pipe->modules & OMAP_ISP_PREVIEW ? printk(" OMAP_ISP_PREVIEW "):__I;
    pipe->modules & OMAP_ISP_RESIZER ? printk(" OMAP_ISP_RESIZER "):__I;
    pipe->modules & OMAP_ISP_AEWB ? printk(" OMAP_ISP_AEWB "):__I;
    pipe->modules & OMAP_ISP_AF ? printk(" OMAP_ISP_AF "):__I;
    pipe->modules & OMAP_ISP_HIST ? printk(" OMAP_ISP_HIST "):__I;
    pipe->modules & OMAP_ISP_CSIARX ? printk(" OMAP_ISP_CSIARX "):__I;
    printk("\n\n");
}
...
static void isp_buf_process(struct device *dev, struct isp_bufs *bufs)
{
    ...
    if ( (old_ick != clk_get_rate(isp->cam_ick)) ||
        (old_mclk != clk_get_rate(isp->cam_mclk)) ||
        (old_dpll4_m5_ck != clk_get_rate(isp->dpll4_m5_ck)) ||
        (old_csi2_96m_fck != clk_get_rate(isp->csi2_fck)) ||
        (old_l3_ick != clk_get_rate(isp->l3_ick)))
    {
        printk("\n\n [%s] DIFF \n\n"
        "old_ick: %lu isp->cal_ick.rate: %lu\n"
        "old_mclk: %lu isp->cam_mclk.rate: %lu\n"
        "old_dpll4_m5_ck: %lu isp->dpll4_m5_ck: %lu\n"
        "old_csi2_96m_fck: %lu isp->csi2_fck.rate: %lu\n"
        "old_l3_ick: %lu isp->l3_ick.rate: %lu\n\n", __func__,
               old_ick, clk_get_rate(isp->cam_ick),
               old_mclk, clk_get_rate(isp->cam_mclk),
               old_dpll4_m5_ck, clk_get_rate(isp->dpll4_m5_ck),
               old_csi2_96m_fck, clk_get_rate(isp->csi2_fck),
               old_l3_ick, clk_get_rate(isp->l3_ick));
...
printk("\n\n [%s] INIT \n\n"
"old_ick: %lu isp->cam_ick.rate: %lu\n"
"old_mclk: %lu isp->cam_mclk.rate: %lu\n"
"old_dpll4_m5_ck: %lu isp->dpll4_m5_ck: %lu\n"
"old_csi2_96m_fck: %lu isp->csi2_fck.rate: %lu\n"
"old_l3_ick: %lu isp->l3_ick.rate: %lu\n\n", __func__,
