...
static void __enable_isp_rsz(struct omap_vout_device *vout, int line)
{
	...
	//printk("#############################the vout->isprsz is 0x%02x ...\n", vout->isprsz);//debug for James
...
//	if (vout->pix.height * vout->pix.width == VID_MAX_WIDTH * 720) {
	//if (vout->pix.height * vout->pix.width >= 480 * 360) {
	/* Henry: Lenovo confirm 480*640 is fine. DSS resizer for low resolution; ISP resizer for high resolution */ 
	if (vout->pix.height * vout->pix.width >= 480 * 360) {
			  ...
			  unsigned int *size)
{
	...
	//enable_isp_rsz(vout); //add by James
	if (rotation_enabled(vout))
...
/*<--LH_SWRD_CL1_James 2011_11_01 CL1 Try to fix the 720p flicker issue while resume*/
		enable_isp_rsz(vout);
/*Try to fix the 720p flicker issue while resume-->*/
...
//&*&*&*20110630_JJ1:CTS test CtsPermissionTestCases
static DEVICE_ATTR(isprsz_enable, S_IRUGO|S_IWUSR|S_IWGRP/*S_IWUGO*/,
		isprsz_enable_show, isprsz_enable_store);
//&*&*&*20110630_JJ2:CTS test CtsPermissionTestCases
//&*&*&*20110629_JJ1:CTS test CtsPermissionTestCases
static DEVICE_ATTR(isprsz_mode, S_IRUGO|S_IWUSR|S_IWGRP/*S_IWUGO*/,
		isprsz_mode_show, isprsz_mode_store);
//&*&*&*20110629_JJ2:CTS test CtsPermissionTestCases
#endif
