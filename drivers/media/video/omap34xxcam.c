...
//unsigned int first_time=0;	//&*&*&*CJ_20110610 for improve captrure performance
				       ...
				       int mask)
{
	...
	for (i = start; i != end + dir; i += dir) {
		...
		/* If is not power off, skip unused sensor. */
		if ((power != V4L2_POWER_OFF) && (i < 2) && (vdev->current_sensor != i)) continue;
				 ...
				 unsigned int *size)
{
	...
	struct omap34xxcam_sensor_config * vsensor_config = (vdev->current_sensor == 0) ? &vdev->vdev_sensor_config0 : 
	                                                                       &vdev->vdev_sensor_config1;
	...
	*size = vdev->pix.sizeimage;
	...
	while (*size * *cnt > vsensor_config->capture_mem)
		(*cnt)--;
			   ...
			   struct v4l2_capability *cap)
{
	...
	struct omap34xxcam_fh *ofh = to_omap34xxcam_fh(vfh);
	struct omap34xxcam_videodev *vdev = ofh->vdev;
	struct v4l2_int_device *vsensor = (vdev->current_sensor == 0) ? vdev->vdev_sensor0 : vdev->vdev_sensor1;
	...
	if (vsensor != v4l2_int_device_dummy())
				   ...
				   struct v4l2_fmtdesc *f)
{
	...
	if (vsensor_config->sensor_isp)
		rval = vidioc_int_enum_fmt_cap(vsensor, f);
	else
			...
			struct v4l2_fract *best_ival)
{
	...
	if (best_ival->numerator == 0
		...
		*best_ival = vsensor_config->ival_default;
	...
	for (fmtd_index = 0; ; fmtd_index++) {
		...
		rval = vidioc_int_enum_fmt_cap(vsensor, &fmtd);
		if (rval)
			break;
		...
		for (size_index = 0; ; size_index++) {
			...
			rval = vidioc_int_enum_framesizes(vsensor,
							  &frms);
			if (rval)
				break;
		      ...
		      struct v4l2_fract *best_ival)
{
	...
	vidioc_int_g_fmt_cap(vsensor, &old_fmt);
	rval = vidioc_int_s_fmt_cap(vsensor, &fmt);
	if (rval)
		return rval;

	a.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
	a.parm.capture.timeperframe = *best_ival;
	rval = vidioc_int_s_parm(vsensor, &a);

	vidioc_int_priv_g_pixclk_active(vsensor, &pixclk);
...
{
	if ((inp->index < 0) || (inp->index > 1))
		return -EINVAL;

	if (inp->index == 0)
		strlcpy(inp->name, "camera0", sizeof(inp->name));
	else
		strlcpy(inp->name, "camera1", sizeof(inp->name));
...
static int vidioc_s_input(struct file *file, void *_fh, unsigned int i)
{
	...
	if(vdev->current_sensor == i) return 0;
		
	mutex_lock(&vdev->mutex);

	/* if camera is streaming, don't switch, return -EBUSY. */
	if (vdev->cam->streaming) {
		rval = -EBUSY;
		goto out;
	}

	/* Power off current camera. */
	omap34xxcam_slave_power_set(vdev, V4L2_POWER_OFF,
					    1 << vdev->current_sensor);
	
	vdev->current_sensor = i;
			    ...
			    struct v4l2_querymenu *a)
{
	...
	if (vsensor_config->sensor_isp)
		return vidioc_int_querymenu(vsensor, a);
			      ...
			      struct v4l2_ext_controls *a)
{
	...
	for (ctrl_idx = 0; ctrl_idx < a->count; ctrl_idx++) {
		...
		if (vsensor_config->sensor_isp) {
			rval = vidioc_int_g_ctrl(vsensor, &ctrl);
		} else {
			for (i = 0; i <= OMAP34XXCAM_SLAVE_FLASH; i++) {
				/* Skip unused sensor. */
				if ((i < 2) && (vdev->current_sensor != i)) continue;
			      ...
			      struct v4l2_ext_controls *a)
{
	...
	for (ctrl_idx = 0; ctrl_idx < a->count; ctrl_idx++) {
		...
		if (vsensor_config->sensor_isp) {
			rval = vidioc_int_s_ctrl(vsensor, &ctrl);
		} else {
			for (i = 0; i <= OMAP34XXCAM_SLAVE_FLASH; i++) {
				/* Skip unused sensor. */
				if ((i < 2) && (vdev->current_sensor != i)) continue;
			 ...
			 struct v4l2_streamparm *a)
{
	...
	mutex_lock(&vdev->mutex);
	rval = vidioc_int_g_parm(vsensor, a);
	mutex_unlock(&vdev->mutex);
...
static int vidioc_cropcap(struct file *file, void *_fh, struct v4l2_cropcap *a)
{
	...
	rval = vidioc_int_cropcap(vsensor, a);

	if (rval && !vsensor_config->sensor_isp) {
		struct v4l2_format f;
		struct v4l2_rect pixel_size;

		/* cropcap failed, try to do this via g_fmt_cap */
		rval = vidioc_int_g_fmt_cap(vsensor, &f);
		if (!rval) {
			isp_g_bounds(isp, &cropcap->bounds);
			cropcap->defrect = cropcap->bounds;
		}

		rval = vidioc_int_priv_g_pixelsize(vsensor,
						   &pixel_size);
		if (!rval) {
...
static int vidioc_g_crop(struct file *file, void *_fh, struct v4l2_crop *a)
{
	...
	printk(KERN_ERR "Enter %s, left = %d, top = %d, width = %d, height = %d. \n", 
	...
	if (vsensor_config->sensor_isp)
		rval = vidioc_int_g_crop(vsensor, a);
	else
		rval = isp_g_crop(isp, a);

	mutex_unlock(&vdev->mutex);
	
	printk(KERN_ERR "Exit %s, left = %d, top = %d, width = %d, height = %d. \n", 
										__func__, a->c.left, a->c.top, a->c.width, a->c.height);
...
static int vidioc_s_crop(struct file *file, void *_fh, struct v4l2_crop *a)
{
	...
	if (vsensor_config->sensor_isp)
		rval = vidioc_int_s_crop(vsensor, a);
	else
				  ...
				  struct v4l2_frmsizeenum *frms)
{
	...
	if (vsensor_config->sensor_isp) {
		rval = vidioc_int_enum_framesizes(vsensor, frms);
		goto done;
	}
				      ...
				      struct v4l2_frmivalenum *frmi)
{
	...
	if (vsensor_config->sensor_isp) {
		rval = vidioc_int_enum_frameintervals(vsensor, frmi);
		goto done;
	}
...
static long vidioc_default(struct file *file, void *_fh, int cmd, void *arg)
{
	...
	if (cmd == VIDIOC_PRIVATE_OMAP34XXCAM_SENSOR_INFO) {
		...
		mutex_lock(&vdev->mutex);
		rval = vidioc_int_priv_g_pixclk(vsensor, &pixclk);
		mutex_unlock(&vdev->mutex);
		if (rval)
			goto out;
		mutex_lock(&vdev->mutex);
		rval = vidioc_int_priv_g_activesize(vsensor,
					    &active_size);
		mutex_unlock(&vdev->mutex);
		if (rval)
			goto out;
		mutex_lock(&vdev->mutex);
		rval = vidioc_int_priv_g_fullsize(vsensor,
						  &full_size);
		mutex_unlock(&vdev->mutex);
		if (rval)
			goto out;
	...
	if (vsensor_config->sensor_isp) {
		...
		case VIDIOC_PRIVATE_ISP_AEWB_REQ:
		{
			...
			if (data->update & SET_EXPOSURE) {
				...
				mutex_lock(&vdev->mutex);
				rval = vidioc_int_s_ctrl(vsensor,
							 &vc);
				mutex_unlock(&vdev->mutex);
				if (rval)
					goto out;
...
static int omap34xxcam_open(struct file *file)
{
	...
	struct v4l2_int_device *vsensor;
	int first_user = 0;
	int i;
	...
	if (atomic_inc_return(&vdev->users) == 1) {			//&*&*&*CJ_20110610 for improve captrure performance
		first_user = 1;
		isp = isp_get();
		if (!isp) {
			rval = -EBUSY;
	...
	vsensor = (vdev->current_sensor == 0) ? vdev->vdev_sensor0 : vdev->vdev_sensor1;

	if (vsensor == v4l2_int_device_dummy() || !first_user)
		goto out_no_pix;

	/* Get the format the sensor is using. */
	rval = vidioc_int_g_fmt_cap(vsensor, &sensor_format);
	if (rval) {
...
static int omap34xxcam_release(struct file *file)
{
	...
	if (atomic_dec_return(&vdev->users) == 0) {		//&*&*&*CJ_20110610 for improve captrure performance
...
static int omap34xxcam_device_register(struct v4l2_int_device *s)
{
	...
	printk(KERN_ERR "%s: Slave %d is register.\n",
			__func__, hwc.dev_type);

	if (hwc.dev_type == OMAP34XXCAM_SLAVE_SENSOR0) {
		isp = isp_get();
		if (!isp) {
			rval = -EBUSY;
	...
	if (hwc.dev_type == OMAP34XXCAM_SLAVE_SENSOR1) {
		isp = isp_get();
		if (!isp) {
			rval = -EBUSY;
			printk(KERN_ERR "%s: can't get ISP, "
	...
	if (hwc.dev_type == OMAP34XXCAM_SLAVE_SENSOR0) {
		...
		rval = vidioc_int_g_fmt_cap(vdev->vdev_sensor0, &format);
		if (rval)
			rval = -EBUSY;
	...
	if (hwc.dev_type == OMAP34XXCAM_SLAVE_SENSOR1) {
		...
		format.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
		rval = vidioc_int_g_fmt_cap(vdev->vdev_sensor1, &format);
		if (rval)
			rval = -EBUSY;
