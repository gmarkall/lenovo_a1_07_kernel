		     ...
		     IN void *pargs)
{
	...
	case BRD_RUNNING:
		dev_context->dw_brd_state = BRD_SLEEP_TRANSITION;
//&*&*&HC1_20110623, change the sequence to avoid accessing register in the wrong step	
//		omap_mbox_save_ctx(dev_context->mbox);
		...
		// insert a delay to wait provious task complete before switching off clock
//		msleep(100);
		
		omap_mbox_save_ctx(dev_context->mbox);	
//&*&*&HC2_20110623, change the sequence to avoid accessing register in the wrong step			
		break;
	case BRD_RETENTION:
		dev_context->dw_brd_state = BRD_SLEEP_TRANSITION;
		omap_mbox_save_ctx(dev_context->mbox);
