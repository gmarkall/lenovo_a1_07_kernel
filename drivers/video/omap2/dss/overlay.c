		...
		const char *buf, size_t size)
{
	...
	printk(" [%s] pos x:%d y:%d\n", __func__, info.pos_x, info.pos_y);
	r = ovl->set_overlay_info(ovl, &info);
	if (r){
        printk(" set overlay info failed %d\n", r);
		return r;
    }
	if (ovl->manager) {
		r = ovl->manager->apply(ovl->manager);
		...
		const char *buf, size_t size)
{
	...
	printk(" [%s] out_w: %d out_h: %d\n", __func__, info.out_width, info.out_height);
	r = ovl->set_overlay_info(ovl, &info);
	if (r){
        printk(" set info failed %d\n", r);
		return r;
