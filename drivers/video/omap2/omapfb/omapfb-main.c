...
#if 0
static void show_logo(struct fb_info *fbi)
{
	int i,j;
	i = 0;
	j = 0;
	...
	tmp = fbi->screen_base;
	memset(tmp, 0, 1024*600*4);
	printk("***************************0x%08x ...\n", fbi->screen_base);
	//memset(tmp, 0, 1024 * 768 * 4);
	printk("***************************%s**************************\n", __func__);
   	for(i = 0; i < 600; i++)
		for(j = 0; j < 1024; j++)
			{
				...
				//tmp = 0x00000000;
				b = gImage_dogs[1024*3*i+3*j] % 256;
				g = gImage_dogs[1024*3*i+3*j+1] % 256;
				r = gImage_dogs[1024*3*i+3*j+2] % 256;
				*tmp = r << 16 | g << 8 | b;
				tmp++;
			}   
	//fill_fb(fbi);
