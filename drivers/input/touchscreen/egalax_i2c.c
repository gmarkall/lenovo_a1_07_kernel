...
#ifdef CONFIG_HAS_EARLYSUSPEND
static void egalax_early_suspend(struct early_suspend *handler);
static void egalax_late_resume(struct early_suspend *handler);
#endif /* CONFIG_HAS_EARLYSUSPEND */

//&*&*&*JJ1_20110623 for NPI read firmware version.
static char touch_firmware_version[4];
...
//&*&*&*BC1_101229:	disable touch wakeup device function
#if 0
...
#endif
//&*&*&*BC2_101229:	disable touch wakeup device function
	return 0;
}
...
static int __devinit egalax_i2c_probe(struct i2c_client *client)
{
	...
	printk("egalax_i2c register input device done\n");
	...
	printk("egalax_i2c request irq(%d) gpio(%d) with result:%d\n", client->irq, gpio, ret);


#ifdef CONFIG_HAS_EARLYSUSPEND
		p_egalax_i2c_dev->early_suspend.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN + 1;
		p_egalax_i2c_dev->early_suspend.suspend = egalax_early_suspend;
		p_egalax_i2c_dev->early_suspend.resume = egalax_late_resume;
		register_early_suspend(&p_egalax_i2c_dev->early_suspend);
#endif /* CONFIG_HAS_EARLYSUSPEND */
...
static void egalax_early_suspend(struct early_suspend *handler)
{
	struct _egalax_i2c *ts;

	ts = container_of(handler, struct _egalax_i2c, early_suspend);
	disable_irq(ts->client->irq);
	printk(KERN_DEBUG "egalax_early_suspend\n");
}
...
static void egalax_late_resume(struct early_suspend *handler)
{
	...
	printk(KERN_DEBUG "egalax_late_resume\n");
}
#endif  /* CONFIG_HAS_EARLYSUSPEND */
