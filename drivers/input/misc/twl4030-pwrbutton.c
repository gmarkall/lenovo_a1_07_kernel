...
static irqreturn_t powerbutton_irq(int irq, void *_pwr)
{
	...
	int err;
	u8 value;
	static u8 pre_event = 0;

	err = twl_i2c_read_u8(TWL4030_MODULE_PM_MASTER, &value,
				STS_HW_CONDITIONS);
	if (!err)  {

		if (!(pre_event ^ (value & PWR_PWRON_IRQ)))
		{
			...
			input_sync(pwr);
			printk("pwr key %d (pair)\n", pre_event);
		}		
		
		input_report_key(pwr, KEY_POWER, value & PWR_PWRON_IRQ);
		input_sync(pwr);
		printk("pwr key %d %d\n", (value & PWR_PWRON_IRQ), pre_event);		
		pre_event = (value & PWR_PWRON_IRQ);
	} else {
...
//&*&*&*HC2_20110706, modify for event pair
