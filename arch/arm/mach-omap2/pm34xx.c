...
//&*&*&*BC1_110601: keep on uart clocks to fix the issue that BT can not pair with other BT device
		if (per_next_state == PWRDM_POWER_OFF) {
			pwrdm_set_next_pwrst(per_pwrdm, PWRDM_POWER_RET);
...
static int sleep_mode_to_poweroff(void)
{
	char buff;
	...
	i2c_read(0x4b,0x2a,1,&buff,1);
	buff = buff | (0x01 << 6);
	i2c_write(0x4b,0x2a,1,&buff,1);
	
	i2c_read(0x4b,0x2b,1,&buff,1);
	buff = buff & ~(0x8 << 0);
	i2c_write(0x4b,0x2b,1,&buff,1);
	
	mdelay(100);
	
	i2c_read(0x4b,0x39,1,&buff,1);
	buff = buff | (1 << 0);
	i2c_write(0x4b,0x39,1,&buff,1);
	
	mdelay(100);
	
	i2c_read(0x4b,0x46,1,&buff,1);
	buff = buff | (1 << 6)|(1 << 0);
	i2c_write(0x4b,0x46,1,&buff,1);
...
static int omap3_pm_suspend(void)
{
	...
	u16 reg;
	char buff;
	u32 i2c_fclk_reg,i2c_iclk_reg;
	u32 irq_status_reg;
	unsigned char battery_capacity[2];
	char wakeup_source;
	u32 i2c_2_scl,i2c_2_sda;
	int read_i2c_flag = 0;;
...
#if 1
		select_bus(0,100);
		i2c_read(0x49,0x81,1,&buff,1);
		g_twl4030_pih_isr = buff;
		buff = buff & ~(0x01 << 5);
		printk("============g_twl4030_pih_isr = 0x%x\n",g_twl4030_pih_isr);
		i2c_write(0x49,0x81,1,&buff,1);
		
		i2c_read(0x4b,0x2e,1,&buff,1);
		wakeup_source = buff;
		if(0 !=	wakeup_source)
			g_twl4030_sih_isr = wakeup_source;
		else
			wakeup_stage=2;
		disable_irq(OMAP_GPIO_IRQ(0));
		printk("========wakeup_source = 0x%x\n",wakeup_source);
		
	if((wakeup_source & TPS65921_PWR_ISR_RTC_IT) \
			&& (0 == get_polling_check_low_battery_flag_android()) \
  		&& (0 != get_polling_check_low_battery_flag_myself()))
  {
			i2c_2_scl = __raw_readw(OMAP2_L4_IO_ADDRESS(0x480021be));
			printk("========i2c_2_scl = 0x%x\n",i2c_2_scl);
	
			i2c_2_sda = __raw_readw(OMAP2_L4_IO_ADDRESS(0x480021c0));
			printk("========i2c_2_sda = 0x%x\n",i2c_2_sda);
			i2c2_init();
			I2c_read(0xaa,0x2c,battery_capacity,2);
			mdelay(100);
			while(read_i2c_flag < 5)
			{
				read_i2c_flag++;
			I2c_read(0xaa,0x2c,battery_capacity,2);
			printk("==========battery_capacity[0] = %d\n",battery_capacity[0]);
			}
	    read_i2c_flag = 0;
		  ...
		  if(battery_capacity[0] > 3)
			{
				...
				compare_android_alarm_to_myself_alarm();
				//gpio1 clear IRQSTATUS1 & IRQSTATUS2
        irq_status_reg = __raw_readl(OMAP2_L4_IO_ADDRESS(0x48310018));
        irq_status_reg  |= (0xffffffff << 0);
        __raw_writel(irq_status_reg,OMAP2_L4_IO_ADDRESS(0x48310018));
        
        irq_status_reg = __raw_readl(OMAP2_L4_IO_ADDRESS(0x48310028));
        irq_status_reg  |= (0xffffffff << 0);
        __raw_writel(irq_status_reg,OMAP2_L4_IO_ADDRESS(0x48310028));
       //PM_WKST_WKUP clear gpio1 wakeup flag
        irq_status_reg = __raw_readl(OMAP2_L4_IO_ADDRESS(0x48306cb0));
      //  irq_status_reg  |= ((0x1 << 3) | (0x1 << 16) | (0x1 << 8));
        irq_status_reg  |= (0x1 << 3);
        __raw_writel(irq_status_reg,OMAP2_L4_IO_ADDRESS(0x48306cb0));
      //PRM_IRQSTATUS_MPU    
        irq_status_reg = __raw_readl(OMAP2_L4_IO_ADDRESS(0x48306818));
      //  irq_status_reg  |= 0xffffffff;
        irq_status_reg  |= 0x1 << 0;
        __raw_writel(irq_status_reg,OMAP2_L4_IO_ADDRESS(0x48306818));
  			mdelay(300);
				goto suspend_again;
			}
			else if(battery_capacity[0] <= 3)
				{
					sleep_mode_to_poweroff();
				}
#endif
		}
