 ...
 * @voltdm:	domain for which to dump the count
 ...
 */
int omap_sr_get_count(struct voltagedomain *voltdm, unsigned int *error,
		unsigned int *count)
{
	struct omap_sr *sr = _sr_lookup(voltdm);
	 ...
	 */
	if (!sr->is_sr_enable) {
		pr_err("%s:%s Clock not enabled! please check sequence\n",
			__func__, voltdm->name);
		return -EINVAL;
	}
