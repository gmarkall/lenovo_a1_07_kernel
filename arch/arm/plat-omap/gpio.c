...
//&*&*&*HC1_20110621, Add to enable gpio wakeup notification for wakeup reason
#ifdef CONFIG_ARCH_OMAP3
		if (cpu_is_omap34xx()) {
			
			// No need to check GPIO group 1
			if (pdev->id != 0) {
				int j = 0;
				for (j = 0; gpio_pads[j].gpio != -1; j++) {
					
					int offset = gpio_pads[j].offset;
					u16 v;
		
					if (gpio_pads[j].gpio < (pdev->id*32))
						continue;	
					
					if (gpio_pads[j].gpio >= ((pdev->id+1)*32) )
						break;					
					...
					// check if WAKEUPEN is set in GPIO_WAKEUPENABLE register
					if (bank->suspend_wakeup & (1 << (gpio_pads[j].gpio%32)))
...
#endif // CONFIG_ARCH_OMAP3		
//&*&*&*HC2_20110621, Add to enable gpio wakeup notification for wakeup reason
...
static void omap_gpio_save_context(struct device *dev)
{
	...
	if (bank->method == METHOD_GPIO_24XX) {
		...
		bank->gpio_context.setwkuena =
			__raw_readl(bank->base + OMAP24XX_GPIO_SETWKUENA);
		bank->gpio_context.setdataout =
...
static void omap_gpio_restore_context(struct device *dev)
{
	...
	if (bank->method == METHOD_GPIO_24XX) {
		...
		__raw_writel(bank->gpio_context.fallingdetect,
				bank->base + OMAP24XX_GPIO_FALLINGDETECT);
		__raw_writel(bank->gpio_context.setwkuena,
				bank->base + OMAP24XX_GPIO_SETWKUENA);
		__raw_writel(bank->gpio_context.setdataout,
